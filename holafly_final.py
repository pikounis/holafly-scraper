...
for entry in links_data:
    ...
    print(f"{counter}. Processing {country} at {url}")
        ...
        if response.status_code == 200:
            ...
            for product_variation in product_variations:
                try:
                    ...
                    print(f"Error processing package for {country} at {url}: {e}")
            ...
            print(f"Failed to fetch the webpage for {country} at {url}: HTTP {response.status_code}")
    except Exception as e:
        print(f"An error occurred while processing {country} at {url}: {e}")

    counter += 1
# Save the extracted data in JSON format to a file named results.json
results_file_path = 'final_results.json'  # Adjust this path as needed
